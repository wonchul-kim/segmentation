...
def evaluate(model, transform, device, num_classes, output_dir):
    ...
    with torch.no_grad():
        ...
        for lens_type in lens_types:
            if not osp.join(output_dir, lens_type):
                os.mkdir(osp.join(output_dir, lens_type))
                os.mkdir(osp.join(output_dir, lens_type, 'over'))
                

            img_files = glob.glob(osp.join(args.data_path, lens_type, '*.jpg'))
            for img_file in tqdm(img_files):
                ...
                cnt += 1
                if torch.sum(preds_labels != 0):
                    output_dir = osp.join(args.output_dir, lens_type, 'over')
                else:
                    output_dir = osp.join(args.output_dir, lens_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-path', 
        default='/home/wonchul/mnt/NAS/Data/01.Image/interojo/3rd_poc/21.11.04_C/양품이미지(21.11.10)')
